...
export default function Editnotes() {
  ...
  const saveChanges = async () => {
    ...
    if (editedUnit !== null && editedUnit !== undefined && editedUnit !== activeUnit) {
    ...
    console.log("Saving note:", { subject, unitKey: activeUnit, updatedFields });

    setIsSaving(true);
    setSaveStatus(null);

    try {
      const response = await updateNote(activeUnit, updatedFields);
      if (response.success) {
          ...
          <div className="flex-1 overflow-y-auto my-scrollable-div">
            <ul className="list-none p-0 m-0 text-left">
              {sortedUnits.map((unit, idx) => {
               const unitKey = `${unit.unit ?? "unit"}-${idx}`;
                ...
                return (
                  <li
                   key={unitKey}
                    onClick={() => handleUnitClick(unitKey)}
